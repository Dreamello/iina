...
and builds with Xcode on an arm64 GitHub Actions runner. Keeping the delta as
standalone patches instead of a fork means there is no branch to keep in sync
with upstream: a build is always *today's* IINA plus the patches, and if a
patch stops applying the build fails loudly and the patch gets fixed.
...
The **Build IINA** workflow runs:

- on every push to `main`
- daily (03:17 UTC), building upstream `develop` — a preflight job skips the
  build when a release for the current upstream commit already exists
